...
function TrackList({ tracks, onTrackSelect, currentTrackId }) {
  ...
  return (
    <div className="flex flex-col">
      <div className="grid grid-cols-[24px_1fr_1fr_56px_32px] gap-4 px-4 py-2 text-taupe uppercase tracking-widest border-b border-white/10 mb-2 font-lcd text-sm">
        <span>#</span>
        <span>Title</span>
        <span>Album</span>
      ...
      {tracks.map((track, index) => {
        ...
        return (
          <div
            key={track.id}
            className={`grid grid-cols-[24px_1fr_1fr_56px_32px] gap-4 px-4 py-2 rounded-md items-center transition-colors group ${
              isActive ? 'bg-panel' : 'hover:bg-white/5'
            }`}
          >
            <button onClick={() => onTrackSelect(track)} className="contents text-left">
              <span className={`font-lcd text-lg ${isActive ? 'text-phosphor [text-shadow:0_0_6px_rgba(255,59,59,0.6)]' : 'text-taupe'}`}>
                {isActive ? '▶' : index + 1}
              </span>

              <span className="flex items-center gap-3 min-w-0">
                <img src={track.cover} alt={track.album} className="w-10 h-10 rounded object-cover flex-shrink-0 border border-black/40" />
                <span className="flex flex-col min-w-0">
                  <span className={`font-medium truncate ${isActive ? 'text-phosphor' : 'text-paper'}`}>{track.title}</span>
